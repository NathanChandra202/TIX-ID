...
@section('content')
    <form action="{{ route('staff.schedule.update', $schedule['id']) }}" class="container my-5" method="POST">
        @csrf
        @method('PATCH')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            @foreach ($schedule['hours'] as $index => $hours)
                ...
                <div class="d-flex align-items-center hour-item">
                    ...
                    {{-- this.closest('.hours-item') : mencari item dengan class hours-item terdekat (diatas component yang di klik i berarti div class hour-item nya) --}}
                    {{-- remove() dihapus komponen --}}
                    @endif
                </div>
